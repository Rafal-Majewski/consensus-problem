...
namespace Numalg {
	...
	void GaussSeidel<DT>::solve(
		...
		std::copy(solution, solution + system.size.variablesCount, newSolution);
		...
		for (int i = 0; i < this->maxIterations; ++i) {
			...
			for (int y = 0; y < system.size.equationsCount; ++y) {
				...
				for (int oi = 0; oi < optimizer[y].size(); ++oi) {
					newSolution[y] -= (*system.coefficients)(y, optimizer[y][oi]) * newSolution[optimizer[y][oi]];
				}
